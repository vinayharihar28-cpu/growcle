...
import { getAdminChaptersList } from '../actions/admin-actions';
...
export function ChapterConfigCard() {
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        <div className="py-8 text-center text-xs text-muted-foreground">Loading chapters...</div>
      ) : chapters.length === 0 ? (
            ...
            <div key={chap.id} className="p-5 rounded-2xl border bg-card space-y-4 shadow-xs hover:border-indigo-500/40 transition-colors">
              <div className="flex items-start justify-between">
                ...
                <span className="px-2.5 py-1 text-[10px] font-bold rounded-full bg-emerald-500/10 text-emerald-600 border border-emerald-500/20">
                  {chap.totalMembersCount || 0} Members
                </span>
              </div>
              ...
              <div className="space-y-2 pt-2 border-t text-xs">
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="p-2 rounded-lg border bg-accent/30">
                    ...
                    <span className="font-bold text-foreground block truncate">{chap.presidentName || 'Unassigned'}</span>
                  </div>
                  <div className="p-2 rounded-lg border bg-accent/30">
                    <span className="text-[10px] text-muted-foreground block font-medium">Vice President</span>
                    <span className="font-bold text-foreground block truncate">{chap.vpName || 'Unassigned'}</span>
                  </div>
                  <div className="p-2 rounded-lg border bg-accent/30">
                    <span className="text-[10px] text-muted-foreground block font-medium">Treasurer</span>
                    <span className="font-bold text-foreground block truncate">{chap.treasurerName || 'Unassigned'}</span>
                  </div>
                </div>
              </div>
